...
<template>
  ...
  >
      ...
      <div class="flex items-center gap-3 text-foreground">
        ...
        <div class="relative flex items-center p-1 bg-muted rounded-lg">
          ...
          <div
            ...
            :style="{
              transform: activeTab === TabTypes.SUMMARY ? 'translateX(100%)' : 'translateX(0)',
            }"
          ></div>

          <!-- Subtitles Button -->
          <button
            @click="selectTab(TabTypes.SUBTITLES)"
            class="relative z-10 p-1 w-7 h-6 flex justify-center items-center"
          ...
          <!-- Summary/CC Button -->
          <button
            @click="selectTab(TabTypes.SUMMARY)"
            class="relative z-10 p-1 w-7 h-6 flex justify-center items-center"
        ...
        <button
          class="p-1 rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"
          @click="openSettings"
        >
    ...
    <main class="flex-grow overflow-y-auto min-h-0">
      <div v-show="activeTab === TabTypes.SUBTITLES" class="p-3">
        <SubtitleViewer
          ...
          v-model:auto-scroll="videoStore.autoScroll.value"
          @jump-to-time="videoStore.jumpToTime"
    ...
    <footer class="p-3 border-t border-border flex-shrink-0">
      <div class="flex items-center gap-2">
        <div
          class="flex-grow h-9 bg-muted rounded-lg flex items-center px-3 cursor-pointer group hover:bg-accent"
        >
          <span class="text-sm text-muted-foreground group-hover:text-accent-foreground"
            >与视频聊天</span
          >
        </div>
        <button class="p-1.5 rounded-lg bg-muted hover:bg-accent group">
          <ChatBubbleLeftIcon
            class="h-5 w-5 text-muted-foreground group-hover:text-accent-foreground"
          />
        </button>
        <button
