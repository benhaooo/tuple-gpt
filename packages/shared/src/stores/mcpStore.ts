...
import type { McpServerConfig, McpServerState, McpConnectionStatus } from '../types/mcp'
...
export const useMcpStore = defineStore(
  ...
  () => {
    ...
    // 监听 servers 变化（storage hydration 或跨页面同步），自动连接新增的 enabled server
    watch(
      servers,
      newServers => {
