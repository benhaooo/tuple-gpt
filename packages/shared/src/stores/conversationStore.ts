...
export const useConversationStore = defineStore('conversations', () => {
  ...
  function addMessage(conversationId: string, message: Omit<ChatMessage, 'id' | 'timestamp'>): ChatMessage {
    ...
    if (conv.title === '新对话' && message.role === 'user' && conv.messages.filter(m => m.role === 'user').length === 1) {
  ...
  function updateMessageStatus(conversationId: string, messageId: string, status: MessageStatus, error?: string) {
    const conv = conversations.value.find(c => c.id === conversationId)
    if (!conv) return
    const msg = conv.messages.find(m => m.id === messageId)
    if (msg) {
