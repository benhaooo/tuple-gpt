    ...
    <div class="mx-3 h-screen min-w-40 flex flex-col transition-all duration-300"
      ...
      <div class="flex items-center h-9 flex-shrink-0 mt-8 overflow-hidden">
        ...
        <div class="ml-4 flex items-center space-x-3">
          <el-tooltip content="删除所有会话" placement="top">
            <TrashIcon @click="handleClearAssistants" class="w-6 h-6 rounded-full cursor-pointer transition-colors
      ...
      <div :class="searchInput || searchFocus ? 'border-primary-500' : 'border-transparent'"
        ...
        <div class="flex w-full bg-surface-light-elevated dark:bg-surface-dark-elevated rounded-md py-1 px-2 border border-border-light-primary dark:border-border-dark-primary">
          <MagnifyingGlassIcon class="w-5 h-5 overflow-hidden text-text-light-secondary dark:text-text-dark-secondary" />
...
import { useWindowSize } from '@shared/hooks/size';
import { getModelLogo } from '@shared/config/model';
import { assistantService } from '@shared/services/AssistantService';
import { TrashIcon, MagnifyingGlassIcon, ChevronLeftIcon, DocumentDuplicateIcon } from '@heroicons/vue/24/outline';
